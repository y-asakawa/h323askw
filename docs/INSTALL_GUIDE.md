...
このドキュメントでは、ソースからローカルでビルドしたH323ASKWの
インストール方法と初期設定について説明します。開発者はソースコードのみを
公開し、DMGやApp Bundleなどのビルド済みファイルは配布しません。
...
ローカルでのビルドにはHomebrewなどの依存関係が必要です。詳細は
[`README.md`](../README.md#building-from-source)を参照してください。
自分で作成したApp Bundleには、スクリプトが依存ライブラリを組み込みます。
...
1. [`README.md`](../README.md#building-from-source)の手順で本体をビルドします。
2. 必要に応じて[App Bundleビルドガイド](APP_BUNDLE_BUILD_GUIDE.md)を使い、
   自分のMac上でBundleを作成します。公開済みソースだけでは、完全な機能を
   持つBundleの再現に必要なプラグインがまだ揃いません。
3. 作成した`H323ASKW.app`を自分のMacの任意の場所に配置します。
